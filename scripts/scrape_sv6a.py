#!/usr/bin/env python3
"""Scrape all SV6a Night Wanderer cards from pokemon-card.com and rebuild data/SV6a.json.

SV6a's IDs are non-contiguous on pokemon-card.com: main set #1–64 lives in one
block, secret rares #65–94 in another. CARD_ID_RANGES captures both. Keep the
ranges in card-number order so the scraper walks the whole set in a single pass.
"""
...
# Confirmed via scripts/probe_sv6a_id_range.py:
#   main set #1–64      → numeric IDs 45876–45939
#   secret rares #65–94 → numeric IDs 46042–46071
CARD_ID_RANGES = [
    (45876, 45939),
    (46042, 46071),
]
...
RARITY_MAP = {
    ...
    "ic_rare_ar_c":  "Illustration Rare",
    "ic_rare_sr":    "Special Illustration Rare",
    "ic_rare_sr_c":  "Special Illustration Rare",
    "ic_rare_sar":   "Special Illustration Rare",
    "ic_rare_sar_c": "Special Illustration Rare",
    "ic_rare_ur":    "Hyper Rare",
    "ic_rare_ur_c":  "Hyper Rare",
...
def main():
    if not CARD_ID_RANGES:
        print("ERROR: CARD_ID_RANGES is empty.")
        print("Run scripts/probe_sv6a_id_range.py and populate this list.")
        sys.exit(2)

    all_cids = []
    for start, end in CARD_ID_RANGES:
        all_cids.extend(range(start, end + 1))
    total = len(all_cids)
    print(f"Fetching {total} SV6a cards from pokemon-card.com across {len(CARD_ID_RANGES)} ranges...")
    results = {}

    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = {executor.submit(parse_card, cid): cid for cid in all_cids}
